...
LMDB_PATH = os.path.join("data", "coc_tensor_10x15x20.lmdb")
...
class CoCEnv(my_gym.Env):
    ...
    def __init__(self, render_mode=None):
        ...
        # Observation space: Box(2) -> [Sharpness, Radius]
        self.observation_space = my_gym.Box(
            low=np.array([0.0, 0.0], dtype=np.float32), 
            high=np.array([float('inf'), 1.0], dtype=np.float32), 
            dtype=np.float32
        )
    ...
    def _get_interpolated_obs(self, val):
        ...
        return np.array([sharpness_obs, radius_obs], dtype=np.float32)
