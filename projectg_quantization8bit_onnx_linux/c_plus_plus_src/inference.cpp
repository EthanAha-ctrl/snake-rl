...
int main(int argc, char *argv[]) {
  ...
  Ort::Session transformer_session(ort_env, transformer_path.c_str(),
                                   session_options);
  ...
  try {
    ...
    while (!done) {
      ...
      std::vector<int64_t> tr_input_shape = {
          1, static_cast<int64_t>(stacked_obs.size())};
      Ort::Value tr_input_tensor = Ort::Value::CreateTensor<float>(
          memory_info, stacked_obs.data(), stacked_obs.size(),
          tr_input_shape.data(), tr_input_shape.size());

      const char *tr_input_names[] = {"obs_stack"};
      const char *tr_output_names[] = {"encoded_features"};

      auto tr_output_tensors =
          transformer_session.Run(Ort::RunOptions{nullptr}, tr_input_names,
      ...
      std::vector<int64_t> act_input_shape = {
          1, static_cast<int64_t>(tr_out_size)};
      Ort::Value act_input_tensor = Ort::Value::CreateTensor<float>(
          memory_info, tr_out_ptr, tr_out_size, act_input_shape.data(),
          act_input_shape.size());

      const char *act_input_names[] = {"features"};
      const char *act_output_names[] = {"action"};

      auto act_output_tensors =
          actor_session.Run(Ort::RunOptions{nullptr}, act_input_names,
                            &act_input_tensor, 1, act_output_names, 1);
      ...
      std::cout << "  Guess: " << action[0] << " | Trigger: " << action[1]
                << " | Reward: [" << step_res.reward[0] << ", "
                << step_res.reward[1] << "]" << std::endl;
    }
