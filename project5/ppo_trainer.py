...
class ActorCritic(nn.Module):
    def __init__(self, input_dim: int, action_dim: int, hidden_dim: int = 128, is_continuous: bool = False):
        super().__init__()
        ...
        if self.is_continuous:
            # For continuous: output Mean. LogStd is a separate learnable parameter.
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.distributions.Distribution, torch.Tensor]:
        ...
        if self.is_continuous:
            ...
            # Use Sigmoid to bound mean to [0, 1] since our env is [0, 1]
            # Actually, standard PPO usually outputs unbounded mean, but for specific range [0,1], 
            # Sigmoid is a valid design choice for the mean.
...
class Transition:
    obs: np.ndarray
    action: float  # support both int and float technically
    log_prob: float
    reward: float
    value: float
    done: bool
...
# ... (kept _explained_variance and _compute_gae same) ...
...
class PPOTrainer:
    def __init__(self, config: TrainConfig, input_dim: int, action_dim: int, is_continuous: bool = False):
        self.cfg = config
        self.device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        self.is_continuous = is_continuous
        self.model = ActorCritic(input_dim=input_dim, action_dim=action_dim, hidden_dim=self.cfg.hidden_dim, is_continuous=is_continuous).to(self.device)
        self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.cfg.lr)
        self.logger = Logger()
        self.n_updates = 0

    def predict(self, obs: np.ndarray) -> Tuple[Union[int, float, np.ndarray], float, float]:
        with torch.no_grad():
            ...
            # Helper for log prob
            if self.is_continuous:
                log_prob = dist.log_prob(action).sum(dim=-1) # Sum over action dim
            else:
        ...
        return action_val, log_prob.item(), value.item()
    ...
    def update_weights(self, transitions: List[Transition]) -> Dict[str, float]:
        ...
        if self.is_continuous:
            action_batch = torch.as_tensor(np.array([t.action for t in transitions]), dtype=torch.float32, device=self.device)
        else:
            action_batch = torch.as_tensor(np.array([t.action for t in transitions]), dtype=torch.int64, device=self.device)
